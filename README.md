...
[Privacy](docs/PRIVACY.md) · [Changelog](CHANGELOG.md) · [License](LICENSE)
...
4. Use **Source Control** for diffs, staging, and commits. Click the branch chip
   ...
   sections. Names like `dima/bugfix/branch-name` appear as nested folders.
   Search matches full paths and expands matching folders. Local branches
   support switch/create/merge/delete; deletion uses `git branch -d`, so
   unmerged work is protected. Clicking a remote branch creates a local tracking
   branch, and its merge action merges the remote ref into your current branch.
   Git refuses to overwrite an existing local branch. Remote branches reflect
   your last fetch; run `git fetch` in a terminal and refresh to update them.
   Emdeck does not fetch automatically.
5. For a text merge conflict, open the file, resolve each block with **Accept
   ours / theirs / both**, review the result, save, stage, and commit. Binary
   and modify/delete conflicts can be handled using Git in a terminal.
...
keep their scroll position without background Git reads. Conflicted and
...
Closing a pane ends its terminal session. Closing a Emdeck window ends only that
window's sessions; other windows stay open. Hiding or maximizing panels keeps
sessions mounted and running. Deliberately detached/background processes can
outlive their shell. Persistent detach/reattach is not implemented. Agent
activity labels are detected from terminal text; connection, recent output,
exit, and error describe the terminal itself.
...
previews are explicitly labeled. See [Release process](docs/RELEASE.md) for
setup and actual validation requirements. Auto-update is not enabled.
...
See [Architecture and repository standards](docs/ARCHITECTURE.md) for ownership,
the Erdos Miller conventions adopted here, dependency rules and validation.
...
This is a beta candidate, not complete WebStorm feature parity. Remote branch
deletion, force push, advanced worktree operations (move/lock/prune), an
extension system, global indexed search, debugger integration and durable agent
session restoration are future work.
...
settings. See [Installation and upgrades](docs/INSTALLATION.md) before removing
an older installation. [Third-party notices](THIRD_PARTY_NOTICES.md) accompany
